...
static void formation_slot(const SwarmState&s,int i,float&sx,float&sy){
 float cx,cy;center_of_mass(s,cx,cy);int n=s.agents.size();
...
void swarm_render_draw(const SwarmState& s){glClear(GL_COLOR_BUFFER_BIT);
 ...
 // Error vectors
 glBegin(GL_LINES);glColor3f(1.f,.3f,.3f);
 for(int i=0;i<s.agents.size();i++){float fx,fy;formation_slot(s,i,fx,fy);auto&a=s.agents[i];
  glVertex2f(nx(a.x),ny(a.y));glVertex2f(nx(fx),ny(fy));}
 glEnd();
